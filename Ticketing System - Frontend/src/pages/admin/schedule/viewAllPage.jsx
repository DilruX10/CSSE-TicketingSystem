...
export default function ViewAllPage() {
	...
	const getSchedules = async () => {
		try {
			const response = await axios.get(
				"http://localhost:8080/api/schedules/view/all"
			);
		...
		if (confirm) {
			try {
				const response = await axios.delete(
					"http://localhost:8080/api/schedules/delete/" + id
				);
		...
		return schedules.map((schedule) => {
			return (
				...
				>
					...
					>
						<Link to={`/viewRoute/${schedule.routeId}`}>
							{schedule.routeId}
						</Link>
					</td>
					<td
						...
						className="p-2 hover:underline  hover:underline-offset-8"
					>
						<Link to={`/viewBus/${schedule.vin}`}>{schedule.vin}</Link>
					</td>
					<td
						align="center"
						className="p-2 hover:underline  hover:underline-offset-8"
					...
					<td align="center" className="w-fit p-2 flex items-center">
						...
						<span
							...
							onClick={(e) => handleDelete(e, schedule.scheduleId)}
						>
	...
	const generatePDF = () => {
		...
		doc.text("Bus Schedule", pageWidth / 2, 60, { align: "center" });
		// Underline the text
		const textWidth =
			(doc.getStringUnitWidth("Bus Schedule") * doc.internal.getFontSize()) /
		...
		// Add table headers
		const headers = ["ID", "Route No,", "Bus", "Departure", "Arrival"];
		...
		// Save PDF file
		doc.save("TBS-Schedule_Table.pdf");
	};
