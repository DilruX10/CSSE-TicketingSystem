...
export default function ViewAllPage() {
		...
		if (confirm) {
			try {
				const response = await axios.delete(
					"http://localhost:8080/api/buses/delete/" + id
				);
		...
		return buses.map((bus) => {
			return (
				...
				>
					...
					>
						<Link to={`/viewDriver/${bus.busDriverId}`}>
							{bus.busDriverId}
						</Link>
					</td>
					{/* Buttons */}
					<td align="center" className="w-fit p-2 flex items-center">
						...
						<span
							...
							onClick={(e) => handleDelete(e, bus.vin)}
						>
	...
	const generatePDF = () => {
		...
		doc.text("Buses Report", pageWidth / 2, 60, { align: "center" });
		// Underline the text
		const textWidth =
			(doc.getStringUnitWidth("Buses Report") * doc.internal.getFontSize()) /
		...
		// Add table headers
		const headers = ["VIN", "Bus Type", "Num Of Seats", "Bus Driver"];
		tableRows.push(headers);

		// Add table data
		buses.forEach((bus) => {
			const data = [bus.vin, bus.busType, bus.numOfSeats, bus.busDriverId];
			tableRows.push(data);
		});
					...
					>
						<thead className="bg-slate-200">
							<tr className="h-8 space-x-4">
								...
								<th className=" ">Num Of Seats</th>
								<th className=" ">Bus Driver</th>
